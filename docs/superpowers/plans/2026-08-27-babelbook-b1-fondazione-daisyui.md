...
Commits: `966cb3d` (T1), `da356fc` (T2), `0e664ab` (T3), `5f4129e` (T4), `63b86aa` (T5), `08436b9` (T6), `f17fbd1` (correzione della tessera), `1d142d5`+`ef04bb7` (ondata finale). Chiusura verificata: typecheck pulito, suite verde (273 core, 330+96 app), e2e 12 superati + 1 saltato, ventidue schermate passate al setaccio. La review finale dell'intero ramo aveva dato BLOCKED per un rilievo Critical — daisyUI portava con sé la mano che punta su bottoni, tab e checkbox, e la scelta di carattere del progetto («il cursore resta la freccia») era sopravvissuta solo sulla carta: la rete dei test guardava il CSS dell'app, cieca a ciò che entra da node_modules. L'ondata finale l'ha risolta con una regola non stratificata (`button.btn, .tab, .checkbox` → freccia; campi disabilitati → freccia; la i dei campi di testo resta la i), ha insegnato al parser e2e a leggere oklch (ed è emerso che il vecchio codice passava per sbaglio: i bottoni disabilitati di daisyUI sono inchiostro al 20% su velature al 10%, e `paints()` ora attraversa le tinte trasparenti), ha rimesso la guardia anti-vacuità al test degli esadecimali e raddrizzato tre commenti. Ogni task ha avuto la sua review; le scelte di giudizio ratificate stanno nei messaggi di commit e qui sotto.
...
- **Correzioni della verifica finale:** la tessera della libreria respirava attaccata al bordo del suo card (imbottitura 0.75rem, come `.terms__warning` e `.group`); la mano che punta e l'oklch, raccontate sopra.
- **Per il futuro (B2/C):** il bordo tratteggiato della voce non servita è invisibile sotto `.btn:disabled`; il warning `initial` del budget Angular (524 kB > 500 kB, con gate d'errore a 1 MB) scende da daisyUI; `.new .input { width: 100% }` è l'unico modulo che sfugge al limite dei 20rem di daisyUI.
